...
test.describe('Booking Module API Tests', () => {
  ...
  test('TC06: Đặt giữ chỗ xem phim thành công (Reserve Seats)', async ({ authRequest }) => {
  ...
  test('TC07: Trùng ghế - Giữ chỗ thất bại khi ghế đã có người giữ (Conflict / Lock)', async ({ authRequest }) => {
  ...
  test('TC08: Webhook thanh toán PayOS thành công chuyển trạng thái CONFIRMED', async ({ request }) => {
  ...
  test('TC09: Kiểm tra Rate Limiting (Spam request)', async ({ request }) => {
